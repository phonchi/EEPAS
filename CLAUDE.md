...
2. **建立專用測試配置檔案**
   - 複製現有配置檔案（如 `config_italy_causal_ew0_rerun.json`）
...
從 v0.4.0 開始，自動邊界調整會：
1. **第一輪**：使用原始 config 檔案（如 `config_italy_causal_ew0_rerun.json`）
2. **如需調整**：創建新的 config 檔案（如 `config_italy_causal_ew0_rerun_autoadjusted_round1.json`）
3. **後續輪次**：使用新的 config 檔案
4. **原始 config 保持不變**：可以重複執行實驗

**範例**：
```bash
# 原始 config: config_italy_causal_ew0_rerun.json
python3 eepas_learning_auto_boundary.py --config config_italy_causal_ew0_rerun.json --three-stage --max-rounds 3

# 如果 Round 1 觸碰邊界：
#   → 創建 config_italy_causal_ew0_rerun_autoadjusted_round1.json（調整後的邊界）
#   → Round 2 使用 config_italy_causal_ew0_rerun_autoadjusted_round1.json
#   → 如再次觸碰邊界，創建 config_italy_causal_ew0_rerun_autoadjusted_round2.json
#   → 原始 config_italy_causal_ew0_rerun.json 保持不變！
```
...
# 測試 Italy 模式
python3 -c "from utils.data_loader import DataLoader; DataLoader.load_catalogs('config_italy_causal_ew0_rerun.json')"
```
